...
def get_secret_or_env(key, default):
    try:
        # Try reading from secrets (looks in .streamlit/secrets.toml relative to script)
        return st.secrets["api"][key]
    except Exception:
        return os.getenv(f"API_{key.upper()}", default)

API_BASE_URL = get_secret_or_env("base_url", "http://localhost:8000")
API_KEY = get_secret_or_env("key", "freelancehub-2024-secret")
...
with st.sidebar:
    ...
    health_data, health_err = safe_api_call(f"{API_BASE_URL}/")
    if health_err:
        ...
        st.json(health_data)
    st.divider()
    st.markdown(f"[📚 API Docs (Swagger)]({API_BASE_URL}/docs)")
...
def get_stats():
    return safe_api_call(f"{API_BASE_URL}/projects/stats")
...
def get_platform_summary():
    return safe_api_call(f"{API_BASE_URL}/projects/platform-summary")
...
with tab1:
    ...
    if st.button("🔎 Fetch Projects", key="filter_btn"):
        fdata, ferr = safe_api_call(f"{API_BASE_URL}/projects/filter", params={"status": status_choice})
        if ferr:
            st.error(ferr)
        else:
...
with tab2:
    ...
    if submitted:
        ...
        pred_data, pred_err = safe_api_call(f"{API_BASE_URL}/predict", method="post", payload=payload)
        if pred_err:
            st.error(pred_err)
        else:
